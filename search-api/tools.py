...
The model expresses AND / OR / NOT over photo sets at two levels:
  ...
  2. ACROSS result sets — combine_results(base, filters, mode) with
     ...
     including across the two separate SQL databases (run_readonly_sql and
     run_readonly_sidecar_sql, see sql_tool.py): combine_results operates on
     handles only, so it doesn't care which tool produced a given handle.
     This is the mechanism that lets a query needing both Immich data and
     sidecar data (e.g. "Kevin's photos of the Eiffel Tower") compose two
     separate database queries without any cross-database JOIN.
...
COMBINE_RESULTS_SCHEMA = {
    ...
    "description": (
        ...
        "operation — use this instead of merging photo ids yourself. Works "
        "across handles from ANY tool (search_photos, run_readonly_sql, "
        "run_readonly_sidecar_sql) — this is how a query needing both the "
        "main database and the sidecar database (e.g. a person AND a "
        "landmark) is composed, since those are separate databases that "
        "can't be joined in one query. base_handle provides the ordering; "
        "each filter handle is a set. mode='union' keeps items in the base "
        "OR any filter (e.g. 'beach photos' plus 'mountain photos'); "
        "mode='intersect' keeps items in the base AND all filters (e.g. "
        "'beach photos' AND 'only-Kevin-in-frame'); mode='subtract' keeps "
        "base items in NONE of the filters. Returns a new handle and count."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
FINALIZE_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "run_readonly_sql, run_readonly_sidecar_sql, or combine_results) and a "
        "short honest explanation of how you got there. IMPORTANT: the "
        "explanation must match the handle you return — do not return one set "
        "while describing another. Do NOT list photo ids; just pass the handle."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
def build_tool_schemas(include_sql=True, include_sidecar_sql=False):
    """
    Assemble the tool schema list sent to the Anthropic API.

    search_photos, combine_results, and finalize_search are always included.
    include_sql toggles run_readonly_sql (Immich's database) so the loop can
    be proven before that tool's Postgres role exists. include_sidecar_sql
    toggles run_readonly_sidecar_sql (the sidecar database) independently —
    defaults to False so callers that don't pass it explicitly (there
    shouldn't be any left, but this is a safe default) don't accidentally
    expose a tool whose DSN may be unset. combine_results works on handles
    from either SQL tool without needing to know which produced them.
    """
    schemas = [SEARCH_PHOTOS_SCHEMA]
    if include_sql:
        from sql_tool import RUN_READONLY_SQL_SCHEMA
        schemas.append(RUN_READONLY_SQL_SCHEMA)
    if include_sidecar_sql:
        from sql_tool import RUN_READONLY_SIDECAR_SQL_SCHEMA
        schemas.append(RUN_READONLY_SIDECAR_SQL_SCHEMA)
    schemas.append(COMBINE_RESULTS_SCHEMA)
    schemas.append(FINALIZE_SEARCH_SCHEMA)
    return schemas
